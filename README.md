...
1. Explain the difference between `RDBMS` and `SQL`.
    `RDBMS` is the way that the database is initially setup and how everything will connect to eachother while `SQL` is used to add content to said database
2. Why do tables need a `primary key`?
    So that they can be easily identified and retrieved if needed.
3. What is the name given to a table column that references the primary key on another table.
    Foriegn key is the name of a column that references another tables primary key.
4. What do we need in order to have a _many to many_ relationship between two tables.
    Both tables would need to have foreign key referencing eachother that and that foreign key can be setup to be unique
## Project Set Up
